...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, patch_n=None, patch_size=None, transform=None):
        ...
        if mode == 'train':
            # 训练集：文件名不以L506开头
            input_ = [f for f in input_path if not os.path.basename(f).startswith(test_patient)]
            target_ = [f for f in target_path if not os.path.basename(f).startswith(test_patient)]
            ...
            # 测试集：文件名以L506开头
            input_ = [f for f in input_path if os.path.basename(f).startswith(test_patient)]
            target_ = [f for f in target_path if os.path.basename(f).startswith(test_patient)]
            if load_mode == 0:
                self.input_ = input_
                self.target_ = target_
            else:
