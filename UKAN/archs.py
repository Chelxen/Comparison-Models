...
class KANBlock2(nn.Module):
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, no_kan=False):
        ...
        self.layer = KANBlock(dim=dim, drop=drop, drop_path=drop_path, norm_layer=norm_layer)#KANLayer(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, no_kan=no_kan)
# =============================================================================
#         self.proj = nn.Conv2d(dim, 256, 3, 2, 1)
#         self.proj2 = nn.Conv2d(dim, 256, 3, 2, 1)
# =============================================================================
        
        #self.patch_embed3 = PatchEmbed(img_size=img_size // 4, patch_size=3, stride=2, in_chans=dim, embed_dim=256)
    ...
    def forward(self, x): #, H, W):
        ...
        #print(x.shape)
        ## KAN layer
        x = x + self.drop_path(self.layer(self.norm2(x), h, w)) ## token space. 
